...
function Navbar() {
  ...
  return (
    <div
      className={`py-1 max-w-screen-2xl container mx-auto md:px-20 dark:bg-slate-900 dark:text-white px-4 fixed top-0 left-0 right-0 z-50 ${
        sticky
    ...
    >
      <div className="navbar">
        <div className="navbar-start">
          <a className="text-2xl font-bold cursor-pointer">
            Book<span className="text-blue-500">Aura</span>
          </a>
        </div>
        <div className="navbar-end">
          ...
          {authUser ? (
            ...
            <div>
              <button
                className="btn mx-2 bg-black text-white hover:bg-slate-800 duration-300"
      ...
      {isModalOpen && (
        ...
        >
          {/* Modal Content */}
          <div
            className="bg-white dark:bg-slate-700 p-6 rounded-md shadow-lg w-96"
            onClick={(e) => e.stopPropagation()} // Prevent closing modal when clicking inside
          >
            <Login /> {/* Insert Login form here */}
          </div>
        </div>
      )}
    </div>
